...
import 'package:tictok_clone2/models/video.dart';
...
class UploadVideoController extends GetxController {
  ...
  Future<String> _uploadVideoToStorage(
    ...
    Reference ref =
        firebaseStorage.ref().child('videos').child(id);
  ...
  Future<String> _uploadImageToStorage(
    ...
    UploadTask uploadTask =
        ref.putFile(await _getThumbnail(videoPath));
    TaskSnapshot snap = await uploadTask;
    String downloadUrl = await snap.ref.getDownloadURL();
    return downloadUrl;
  }

  uploadVideo(String songName, String caption,
      File videoPath) async {
    try {
      ...
      String videoUrl = await _uploadVideoToStorage(
          "video $len", videoPath.path);
      String thumbnail = await _uploadImageToStorage(
          'video $len', videoPath.path);
      print("the user doc ${userDoc.data}");
      Video video = Video(
